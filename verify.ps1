...
# previous update is present in BOTH your source files AND story_v2.html.
# If anything fails, do not push — investigate which file is stale.
#
...
# Each check: file path, regex pattern, minimum expected count, version, description
$checks = @(
    # === Engine (v20 / v23) ===
    ...
    # === EPA insults & verbatim text (v19, v20) ===
    ...
    @{ File='src/data/scenes/epa.js';      Pattern="world's supply";                      Min=1; Ver='v20'; Desc="grammar — world's apostrophe" },
...
foreach ($c in $checks) {
    ...
    $matches = (Select-String -Path $file -Pattern $pat -AllMatches).Matches.Count
    if ($matches -ge $min) {
        Write-Host ("  OK [{0}] {1,-32} /{2}/ => {3} (>= {4}) -- {5}" -f $ver, $file, $pat, $matches, $min, $desc) -ForegroundColor Green
        $pass++
    } else {
        Write-Host ("  X  [{0}] {1,-32} /{2}/ => {3} (NEED >= {4}) -- {5}" -f $ver, $file, $pat, $matches, $min, $desc) -ForegroundColor Red
        $fail++
...
if ($fail -gt 0) {
    ...
    Write-Host ""
    Write-Host "Do NOT push. Re-extract the corresponding zip drop and re-run 'node build.js'." -ForegroundColor Yellow
    exit 1
}
